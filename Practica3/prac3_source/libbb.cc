...
void Equilibrado_Carga(tPila &pila, bool &fin, tNodo &sol)
{
  ...
  //Necesitamos pedir trabajo porque no nos queda
  if (pila.vacia())
  {

    MPI_Send(&rank, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);

    //Mientras esperamos a que recibir trabajo
    while (pila.vacia() && !fin)
    {
      //recibimos algun tipo de mensaje
      MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &status);

      tipoMensaje = status.MPI_TAG;
      ...
      case PETICION:
        ...
        if (solicitante == rank)
        {
          ...
          if (token_presente)
          {
            //En caso de que no seamos el primer proceso enviamos nuestro color
            if (rank == 0)
              color_token = BLANCO;
            else if (color == NEGRO) //Si somos el primer proceso enviamos el color blanco si o si
              color_token = NEGRO;

            //Se envia el token hasta que lo volvamos a recibir.
        ...
        {
          // enviamos la solicitud al siguiente proceso
          MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
        }
        break;
      ...
      case NODOS: //Resultado de una petición de trabajo
        ...
        //vemos cuantos vamos a recibir
        MPI_Get_count(&status, MPI_INT, &cantidad);
        //Vemos de donde vamos a recibir y recibimos
        fuente = status.MPI_SOURCE;
        MPI_Recv(pila.nodos, cantidad, MPI_INT, fuente, NODOS, comunicadorCarga, &status);
      ...
      //TODO
      case TOKEN:
        // recibimos el token y ponemos que lo tenemos
        MPI_Recv(NULL, 0, MPI_INT, siguiente, TOKEN, comunicadorCarga, &status);
        token_presente = true;

        //Si  estamos en modo pasivo
        if (estado == PASIVO)
        {
          ...
          if (rank == 0 && color == BLANCO && color_token == BLANCO)
          {
            ...
            fin = true;
            // Se envia el fin al siguiente
            MPI_Send(sol.datos, 2 * NCIUDADES, MPI_INT, siguiente, FIN, comunicadorCarga);
            // Espera recibir el fin del anterior (habiendo pasado por todos antes)
            MPI_Recv(aux.datos, 2 * NCIUDADES, MPI_INT, anterior, FIN, comunicadorCarga, &status);
            // Nos quedamos con la mejor solución de todas
            if (aux.ci() < sol.ci())
              CopiaNodo(&aux, &sol);
          }
          else
          { //EN caso de que no sea el fin
            if (rank == 0)
              color_token = BLANCO;
            else if (color == NEGRO)
              color_token = NEGRO;
            //Envia el token al  anterior.
            MPI_Send(NULL, 0, MPI_INT, anterior, TOKEN, comunicadorCarga);
            token_presente = false;
            color = BLANCO;
          }
      ...
      case FIN:
        fin = true;
        //Recibimos la solución
        MPI_Recv(aux.datos, 2 * NCIUDADES, MPI_INT, anterior, FIN, comunicadorCarga, &status);
        // Nos quedamos con la mejor solucción
        if (aux.ci() < sol.ci())
          CopiaNodo(&aux, &sol);
        //Enviamos el token fin y la solucción al siguiente
        MPI_Send(sol.datos, 2 * NCIUDADES, MPI_INT, siguiente, FIN, comunicadorCarga);
        break;
      }
    }
  ...
  { // el proceso tiene nodos para trabajar
    ...
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &cantMensajes, &status);
    ...
    { // atiende peticiones mientras haya mensajes
      ...
      switch (tipoMensaje)
      {
      case PETICION: //piden trabajo
        MPI_Recv(&solicitante, 1, MPI_INT, anterior, PETICION, comunicadorCarga, &status);

        if (pila.tamanio() > 1)
        { // hay suficientes nodos en la pila para ceder
          tPila pilaAuxiliar;
          pila.divide(pilaAuxiliar);
          // Enviamos nodos al proceso solicitante
          MPI_Send(pilaAuxiliar.nodos, pilaAuxiliar.tope,
                   MPI_INT, solicitante, NODOS, comunicadorCarga);

          // Reactivamos el problema
          if (rank < solicitante)
            color = NEGRO;
        }
        else
        {
          //no tenemos trabajo y pasamos petición al proceso siguiente
          MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
        }
        break;

      case TOKEN: //Recibe el token
...
void Difusion_Cota_Superior(int &cuota_actual, bool &actualizar)
{
  pendiente_retorno_cs = false;
  difundir_cs_local = actualizar;
  MPI_Status status;

  if (difundir_cs_local && !pendiente_retorno_cs)
  {
    MPI_Send(&cuota_actual, 1, MPI_INT, siguiente, rank, comunicadorCota);
    pendiente_retorno_cs = true;
    difundir_cs_local = false;
  }
  int origen, cantidad;
  //Sondear si hay mensajes de cota superior pendientes;
  MPI_Iprobe(anterior, MPI_ANY_TAG, comunicadorCota, &cantidad, &status);
  origen = status.MPI_TAG;
  int cota_nueva;
  while (cantidad > 0)
  { //hay mensajes
    //Recibir mensaje con valor de cota superior desde el proceso (id-1+P)%P
    MPI_Recv(&cota_nueva, 1, MPI_INT, anterior, origen, comunicadorCota, &status);
    //Actualizar valor local de cota superior;
    if (cota_nueva < cuota_actual)
    {
      actualizar = true;
      cuota_actual = cota_nueva;
    }
    ...
    {
      MPI_Send(&cuota_actual, 1, MPI_INT, siguiente, origen, comunicadorCota);
    }
    //Sondear si hay mensajes de cota superior pendientes;
    MPI_Iprobe(anterior, MPI_ANY_TAG, comunicadorCota, &cantidad, &status);
    origen = status.MPI_TAG;
