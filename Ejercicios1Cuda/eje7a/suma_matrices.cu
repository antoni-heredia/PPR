...
int index=i*N; // Compute global 1D index
if (i < N)
  for(int x = 0; x < N;x++)
  	C[index+x] = A[index+x] + B[index+x]; // Compute C element
}
...
for (i=0; i<NN;i++)
{
  A[i]= (float) i;
  B[i]= -(float) i;
  C[i] = 1.0;//Para comprobar que se cambiar el valor en C
}
...
int numBlocks = ceil((NN)/threadsPerBlock);
