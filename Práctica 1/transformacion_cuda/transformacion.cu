...
//**************************************************************************
{
  ...
  transformacion_compartida<<<blockNum,blockSize, Bsize*2*sizeof(float)>>>(A_device,B_device,C_device, N);
  ...
  err = cudaMemcpy(C, C_device, size, cudaMemcpyDeviceToHost);
	if (err != cudaSuccess) {
